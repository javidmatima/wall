...
function InitialSetup({ onEnterApp }: { onEnterApp: () => void }) {
  ...
  const [unavailableFundingMethod, setUnavailableFundingMethod] = useState<string | null>(null)
  ...
  useEffect(() => {
    if (!unavailableFundingMethod) return
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setUnavailableFundingMethod(null)
    }
    window.addEventListener('keydown', closeOnEscape)
    return () => window.removeEventListener('keydown', closeOnEscape)
  }, [unavailableFundingMethod])
...
const beginAppEntry = () => setStep('entering-app')
  ...
  if (step === 'fund-wallet') return <main className="initial-setup fund-wallet-screen">
    <header className="fund-wallet-header"><button type="button" onClick={beginAppEntry} aria-label="Back"><BackArrowIcon /></button><h1>Fund your wallet</h1><span /></header>
    <section className="fund-wallet-section"><h2>Recommended for you</h2><button type="button" className="fund-wallet-row" onClick={() => setUnavailableFundingMethod('Google Pay')}><span className="fund-google-pay">G Pay</span><strong>Google Pay</strong><b>›</b></button></section>
    <section className="fund-wallet-section"><h2>All options</h2><div className="fund-wallet-options">
      <button type="button" className="fund-wallet-row" onClick={() => setUnavailableFundingMethod('All payment methods')}><span className="fund-option-icon"><Icon name="wallet" size={22} /></span><strong>All payment methods</strong><b>›</b></button>
      <button type="button" className="fund-wallet-row" onClick={() => setUnavailableFundingMethod('Exchange')}><span className="fund-option-icon"><Icon name="swap" size={22} /></span><strong>Exchange</strong><b>›</b></button>
      <button type="button" className="fund-wallet-row" onClick={() => setUnavailableFundingMethod('Crypto wallet')}><span className="fund-option-icon"><Icon name="qr" size={22} /></span><strong>Crypto wallet</strong><b>›</b></button>
    </div></section>
    {unavailableFundingMethod && <div className="fund-unavailable-overlay" role="presentation" onClick={() => setUnavailableFundingMethod(null)}><section className="fund-unavailable-dialog" role="dialog" aria-modal="true" aria-labelledby="fund-unavailable-title" aria-describedby="fund-unavailable-description" onClick={(event) => event.stopPropagation()}><span className="fund-unavailable-icon"><Icon name="info" size={30} /></span><h2 id="fund-unavailable-title">{unavailableFundingMethod} is currently unavailable</h2><p id="fund-unavailable-description">This funding method isn&apos;t available right now. Please try another option or check back later.</p><button type="button" autoFocus onClick={() => setUnavailableFundingMethod(null)}>Got it</button></section></div>}
  </main>
  ...
  return (
    <main className="app-shell">
      <div className={`wallet-app${activeTab === 'markets' ? ' markets-wallet-app' : ''}`}>
        ...
        {uiFeatureConfig.showHomeBalanceActions && usdtValue > 0 && <section className="home-balance-section" aria-labelledby="home-balance-title">
